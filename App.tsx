...
export default function App() {
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const consolidatedResult = await aiService.consolidateDetections(
        detectionResult,
        selectedFile
      );
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "An error occurred while detecting text. Please try again.";
  ...
  const handleDecompose = async () => {
    ...
    try {
      ...
      const hasProperties = textDetections.every(
        (d) => d.fontSize !== undefined
      );
      ...
      if (!hasProperties && previewUrl) {
        ...
        const propertyPromises = textDetections.map(
          async (detection, index) => {
            try {
              const properties = await aiService.detectFontProperties(
                detection,
                selectedFile,
                naturalWidth,
                naturalHeight
              ...
              console.log(
                `[VLM Property Detection] Box ${index} ("${detection.label}"):`,
                properties
              );
              return properties;
            } catch (err) {
              console.error(
                `Error detecting properties for box ${index}:`,
                err
              );
              return {
                fontSize: 16,
      ...
      const cleanedImageUrl = await aiService.removeTextFromImage(
        selectedFile,
        currentDetections
      );
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "An error occurred during decomposition. Please try again.";
      setError(errorMessage);
      setStep("result");
    } finally {
      setIsDecomposing(false);
          ...
          {!selectedFile ? (
            ...
            >
              ...
              <p className="text-lg text-gray-500">
                Upload an image for decomposition
              </p>
              <input
                id="file-upload"
                name="file-upload"
            ...
            <div className="flex flex-col items-center gap-6">
              <div className="relative w-full max-w-md">
                ...
                />
                {imageRenderedSize.width > 0 &&
                  step === "result" &&
                  textDetections.map((boxData, index) => (
                    <OverlayComponent
                      key={index}
                      boxData={boxData}
